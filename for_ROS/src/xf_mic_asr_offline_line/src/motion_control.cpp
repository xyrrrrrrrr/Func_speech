...
#include <xf_mic_asr_offline_line/position.h>
#include <string.h>
#include <math.h>
#include <joint.h>
#include <iostream>
...
返回  值：无
**************************************************************************/
void current_position_Callback(const xf_mic_asr_offline_line::position& msg)	
{
...
void goal_reach_Callback(const move_base_msgs::MoveBaseActionResult& msg)
{
	...
	{
		system("aplay -D plughw:CARD=Device,DEV=0 ~/wheeltec_robot/src/xf_mic_asr_offline_line/feedback_voice/reach_goal.wav");
		goal_control=0;
	}
	else if(reach == "Goal reached." && overmap_flag==1)//建图完成播报
	{
		system("aplay -D plughw:CARD=Device,DEV=0 ~/wheeltec_robot/src/xf_mic_asr_offline_line/feedback_voice/overmap.wav");
		overmap_flag=0;
	}
...
//fangxiang
	...
	goal_angle_r= turn_angle /180*3.1415926;
...
int main(int argc, char** argv)
{
  ...
  /***创建障碍物方位话题订阅者***/
	...
	//ros::ServiceClient Set_Major_Mic_client = node.serviceClient<xf_mic_asr_offline_line::Set_Major_Mic_srv>("xf_asr_offline_node/set_major_mic_srv");
	...
	//xf_mic_asr_offline_line::Set_Major_Mic_srv num;
		...
		if(cmd_vel_flag || turn_fin_flag)    //底盘运动控制部分
		{
			...
			if(distance_judgment() && dis_angleX_judgment())    //判断障碍物的距离和方向
			{
				...
				if(temp_count > radar_count)    //连续计数5️次停止运动防止碰撞，避免雷达有噪点
				{
					...
					turn_fin_flag = 0;
					temp_count = 0;
					system("aplay -D plughw:CARD=Device,DEV=0 ~/wheeltec_robot/src/xf_mic_asr_offline_line/feedback_voice/find.wav");
					}
				}
			}
